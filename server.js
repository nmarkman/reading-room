...
const BOOKS_CSV_PATH = join(ROOT, "data", "lindsay-goodreads-read.csv");
const ANTHROPIC_VERSION = "2023-06-01";
const ANTHROPIC_MODEL = process.env.ANTHROPIC_MODEL || "claude-sonnet-5";
const FOLLOWUP_MODEL = process.env.ANTHROPIC_FOLLOWUP_MODEL || "claude-haiku-4-5-20251001";
const OP_ITEM_ID = "nnr3ybwtvbdbv24s2z24cwx3cm";
const OP_VAULT = "MarkClaw";
...
function getAnthropicKey() {
  ...
  const token = execFileSync("cat", [expandHome("~/.config/op/service-account-token")], {
    encoding: "utf8",
    stdio: ["ignore", "pipe", "ignore"],
  }).trim();

  cachedKey = execFileSync(
    "op",
    ["item", "get", OP_ITEM_ID, "--vault", OP_VAULT, "--fields", "credential", "--reveal"],
    {
      encoding: "utf8",
...
async function loadBooks() {
  ...
  const csv = await readFile(BOOKS_CSV_PATH, "utf8");
  cachedBooks = parseCsv(csv).map(normalizeBook).filter((book) => book.title);
  return cachedBooks;
}
...
function buildPrompt(action, books, question, history) {
  ...
  const stats = summarizeBooks(books);
  ...
  return [
    ...
    "Do not mention technical implementation details, spreadsheets, scraping, schemas, field names, JSON, columns, features, or internal labels.",
    ...
    `Shelf stats JSON: ${JSON.stringify(stats)}`,
...
function summarizeBooks(books) {
  ...
  const ratings = books.reduce((counts, book) => {
...
function sessionHistory(sessionId) {
  const id = cleanDisplayText(sessionId || "default").slice(0, 120) || "default";
...
function extractToolInput(message) {
  ...
  if (!text) throw new Error("Claude returned no insight.");
  return normalizeInsight({
    title: "Shelf insight",
    summary: smartTrim(text, 520),
    bullets: [],
    actions: ["Try a different AI action"],
  });
}
...
function normalizeListField(value) {
  if (Array.isArray(value)) {
    return value.flatMap((item) => normalizeListField(item));
  }
  if (!value) return [];
...
function extractFollowups(message, fallback) {
  const block = (message.content || []).find(
    (item) => item.type === "tool_use" && item.name === "render_followups",
  );
  const followups = block?.input?.followups;
  if (Array.isArray(followups) && followups.length) {
    return followups.map((item) => String(item).slice(0, 80)).slice(0, 4);
  }
  return fallback;
}
...
async function handleAi(req, res) {
  try {
    ...
    const sessionId = String(body.sessionId || "default").slice(0, 160);
    ...
    if (!response.ok) {
      json(res, response.status, { error: "Anthropic request failed.", detail: safeError(raw) });
      return;
    }
...
async function serveStatic(req, res) {
  ...
  if (
    ...
    (!publicPaths.includes(requested) && !requested.startsWith("/data/"))
  ) {
    res.writeHead(403);
    res.end("Forbidden");
    return;
...
createServer((req, res) => {
  ...
  console.log(`Reading Room server listening on http://0.0.0.0:${PORT}`);
});
